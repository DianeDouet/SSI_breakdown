...
function recombination(par, L)
    ...
    return (Sallele_chr1, Sallele_chr2, [], []) #Changed for tetraploids
end
...
function dichotomie_delta(delta, deltaMax, deltaMin, freqSC)
    #if freqSC < 1 #Threshold between coexistence/invasion of the SC allele at equilibrium 
    if freqSC < 0.05 #Threshold between maintenance/absence of the SC allele at equilibrium
        deltaMax = delta
    else
        deltaMin = delta
    end
