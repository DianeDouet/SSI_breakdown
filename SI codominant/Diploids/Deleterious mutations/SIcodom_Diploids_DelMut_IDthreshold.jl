...
function recombination(par, L)
    ...
    sort!(pos_crossovers)
    #print("positions = ", pos_crossovers)
...
function dichotomie(U,Umin,Umax, freqSC)
    if freqSC < 0.05 
        Umax = U
    else
        Umin = U
...
function simulation(nbSim, Npop, h,s, alpha, U, U_SI, U_SC, L, dominance)
    ...
    while Umax-Umin > 0.01 && Umin < 5.91
        nbSalleles = 0
        nbSCalleles = 0
        freqSC = 0.0
        ID = 0
        ...
        #Dichotomie
        U, Umin, Umax = dichotomie(U,Umin,Umax, freqSC)
    end
...
h = 0.25
s= 0.05
alpha = 0.1 #Tested from 0 to 1
U_SC = 1e-4
U_SI = 1e-5
U = 5 #To change in order to control Inbreeding depression
dom = 1 #dominance of SC allele (0 if recessive or codominant, 1 if dominant  over the other SI alleles)



Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_U = simulation(nbSim, Npop, h,s, alpha, U, U_SI, U_SC, L, dom)
