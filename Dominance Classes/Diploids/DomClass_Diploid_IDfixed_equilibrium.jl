using Distributions
using StatsBase



#Mutations Si to Sj 
function mutation_SI!(Npop, list_ind, U_SI, k)
    nbMut = rand(Binomial(2*Npop,U_SI)) #number of mutations among all S alleles
    for _ in 1:nbMut
        indMut = rand(1:Npop) #choosing the individual
        #choosing the allele of that individual that will mutate
        chrMut = rand(1:2)
        allMut = list_ind[indMut][1][chrMut] #S allele that will mutate
        if allMut != -1 #mutates only if the allele is SI
            mutant_S = rand(0:k-1) #Choosing a random S allele to replace the existing one
            while mutant_S == allMut #Making sure that the new S allele is different from the old one
                mutant_S = rand(0:k-1)
            end
            list_ind[indMut][1][chrMut] = mutant_S #Replacing with mutant S allele
        end
    end
end


#Mutations Si to SC
function mutation_SC!(Npop, list_ind, U_SC)
    nbMut = rand(Binomial(2*Npop,U_SC)) #number of mutations among all S alleles
    for _ in 1:nbMut
        indMut = rand(1:Npop) #choosing the individual
        #chossing the allele of that individual that will mutate
        chrMut = rand(1:2)
        allMut = list_ind[indMut][1][chrMut]
        if allMut != -1 #mutates only if the allele is SI
            list_ind[indMut][1][chrMut] = -1 #Replacing with mutant SC allele
        end
    end
end




## RECOMBINATION 

function recombination(par, L)
    #number and positions of the crossover
    nb_crossovers = rand(Poisson(L))
    pos_crossovers = rand(nb_crossovers)
    sort!(pos_crossovers)

    #recombined chromosomes
    chr1 = par[2]
    chr2 = par[3]
    Sallele1 = par[1][1]
    Sallele2 = par[1][2]
    #shuffling the two chromosomes
    if rand() < 0.5
        chr1,chr2 = chr2,chr1
        Sallele1,Sallele2 = Sallele2,Sallele1
    end

    gamete::Vector{Float64} = []
    sizehint!(gamete, floor((length(chr1)+length(chr2))/2)) #Suggest that gamete reserve some capacity in advance. This can improve performance.
    Sallele = Sallele1

    first_possible_index_chr1 = 1
    first_possible_index_chr2 = 1
    
    for i in 1:nb_crossovers + 1
        #Looking between two crossovers
        if nb_crossovers == 0
            j=0
            k=1
        elseif i == 1
            j = 0
            k = pos_crossovers[i]
        elseif i == nb_crossovers +1
            j = pos_crossovers[i-1]
            k = 1
        else
            j = pos_crossovers[i-1]
            k = pos_crossovers[i]
        end

        #Take on the first chromosome
        if i % 2 == 0
            if j <= 0.5 && k >=0.5
                Sallele = Sallele1
            end
            for l in first_possible_index_chr1:length(chr1)
                if chr1[l] >= k
                    first_possible_index_chr1 = l
                    break
                end
                if chr1[l] >= j
                    push!(gamete,chr1[l])
                end
            end
        end

        #Take on the second chromosome
        if i%2 != 0
            if j <= 0.5 && k >=0.5
                Sallele = Sallele2
            end
            for l in first_possible_index_chr2:length(chr2)
                if chr2[l] >= k
                    first_possible_index_chr2 = l
                    break
                end
                if chr2[l] >= j
                    push!(gamete,chr2[l])
                end
            end
        end
        
    end

    return (Sallele, gamete)
end




## SELFING RATE 

function selfing_rate(Npop, list_ind, par, alpha, list_W, dominanceSC)
    Salleles = list_ind[par][1] #list of the first parent's S-alleles

    SR = 0.0
    
    #Special cases
    if Salleles[1] != -1 && Salleles[2] != -1 #the individual is SI (two SI alleles)
        return 0.0
    elseif Salleles[1] == -1 && Salleles[2] == -1 ##Two SC alleles, compatible with everyone
        gamma_ii = 2
        gamma_ij = 2
        sum_val = gamma_ij*(sum(list_W)-list_W[par])
        SR = (alpha*list_W[par]*gamma_ii)/(alpha*list_W[par]*gamma_ii + ((1-alpha)/(Npop-1))*sum_val)
    
        
    #Case 1 SC
    else
        
        Si = 0
        
        #Dominance for selfing (mother's allele)
        if Salleles[1] != -1 #The first S allele is SI
            domM_SI = Salleles[1]÷25 + 1 #Dominance class of the SI S allele
            Si = Salleles[1]
        else #The second S allele is SI 
            domM_SI = Salleles[2]÷25 + 1 
            Si = Salleles[2]
        end
        
        
        if dominanceSC <= domM_SI #SC is codominant or recessive
            return 0.0
            
        else #SC is dominant
            gamma_ii = 2
            
            sum_val = 0
            for j in 1:Npop
                gamma_ij = 0
                if j != par
                    Sj = list_ind[j][1] #S alleles of the pollen
                    Sj_1 = Sj[1]
                    Sj_2 = Sj[2]
                    
                    #Dominance in the pollen
                    domF_1 = Sj_1÷25 + 1 #Dominance class of the first allele in the pollen
                    domF_2 = Sj_2÷25 + 1 #Dominance class of the second allele in the pollen
                    if Sj_1 == -1
                        domF_1 = dominanceSC
                    end
                    if Sj_2 == -1
                        domF_2 = dominanceSC
                    end
                    #We artificially consider that both S alleles in the father are the most dominant one
                    if domF_1 > domF_2
                        Sj_2 = Sj_1
                    end
                    if domF_2 > domF_1
                        Sj_1 = Sj_2
                    end
                    
                    
                    #Checking the compatibility between individuals i and j
                    if Sj_1 == Si || Sj_2 == Si #One of the pollen's S allele is the same that the pistil's SI allele
                        gamma_ij = 0
                    else #All pollen's S alleles are different from the pistil's SI allele
                        gamma_ij = 2
                    end
                end
		sum_val += list_W[j]*gamma_ij
            end
            
                
            SR = (alpha*list_W[par]*gamma_ii)/(alpha*list_W[par]*gamma_ii + ((1-alpha)/(Npop-1))*sum_val)
        end
    end

    return SR
end




## REPRODUCTION 

function reproduction!(Npop, list_ind, list_W, list_off, list_W_off, alpha, delta, L, phase, dominanceSC)
    fitnessMax = maximum(list_W)

    Npop_off = 0

    while Npop_off != Npop

        # Selecting a mother
        par1 = rand(1:Npop) #Selecting a parent randomly
        fitnessPar1 = list_W[par1]

        while fitnessPar1/fitnessMax < rand() #A new maternal parent is selected if the fitness of the first selected parent is too low
            par1 = rand(1:Npop)
            fitnessPar1 = list_W[par1]
        end
        

        Salleles_par1 = list_ind[par1][1]
        Sallele_par1_1 = Salleles_par1[1]
        Sallele_par1_2 = Salleles_par1[2]
        


        #Selfing or outcrossing ?
        selfRate = selfing_rate(Npop, list_ind, par1, alpha, list_W, dominanceSC)
        fit_off = 1.0

        #OUTCROSSING
        if selfRate <=  rand()
            OK_father = false
            while OK_father == false
                #Selecting a father
                par2 = rand(1:Npop) #A second parent is selected
                fitnessPar2 = list_W[par2]
                          
                
                #Checking the father
                if par2 == par1 # Checking that the father is different from the mother
                    continue
                end
                if fitnessPar2/fitnessMax < rand() # Checking its fitness
                    continue
                end
                
                
                # Checking the compatibility at the S-locus
                
                Salleles_par2 = list_ind[par2][1]
                Salleles_par2_1 = Salleles_par2[1]
                Salleles_par2_2 = Salleles_par2[2]
                
                #Dominance in the father
                domF_1 = Salleles_par2_1÷25 + 1
                domF_2 = Salleles_par2_2÷25 + 1
                if Salleles_par2_1 == -1
                    domF_1 = dominanceSC
                end
                if Salleles_par2_2 == -1
                    domF_2 = dominanceSC
                end
                #We artificially consider that both S alleles in the father are the most domi
                if domF_1 > domF_2
                    Salleles_par2_2 = Salleles_par2_1
                end
                if domF_2 > domF_1
                    Salleles_par2_1 = Salleles_par2_2
                end
                
                
                #Checking the compatibility of the pollen
                
                if Salleles_par2_1 == -1 && Salleles_par2_2 == -1 #Both alleles in the pollen is SC
                        OK_father = true
                        break
                else #1 or 0 SC alleles
                    if Salleles_par2_1 == -1 #One SC allele
                        if Salleles_par2_2 != Salleles_par1[1] && Salleles_par2_2 != Salleles_par1[2] #Check if the second is compatible
                            OK_father = true
                            break
                        end
                    elseif Salleles_par2_2 == -1 #One S allele
                        if Salleles_par2_1 != Salleles_par1[1] && Salleles_par2_1 != Salleles_par1[2] #Check if the first is compatible
                            OK_father = true
                            break
                        end
                    else #2 SI alleles
                        if Salleles_par1[1] != Salleles_par2_1 && Salleles_par1[1] != Salleles_par2_2 && Salleles_par1[2] != Salleles_par2_1 && Salleles_par1[2] != Salleles_par2_2  
                            OK_father = true
                            break
                        end
                    end
                end
            end
        
        #SELFING
        #All pollen grains are compatible when SC
        else
            par2 = par1
            fit_off = 1-delta
        end           

         
        #Recombination
        Sallele1, chr1 = recombination(list_ind[par1], L)
        Sallele2, chr2 = recombination(list_ind[par2], L)
        #Creating a new individual for the next generation
        Npop_off += 1
        offspring = [[Sallele1, Sallele2], chr1, chr2]
        list_off[Npop_off] = offspring
        list_W_off[Npop_off] = fit_off
    end
end



## MAIN 

#Phases:
# - Only mutations Si to Sj (2000 generations)
# - Then mutations on the genome (2000 generations)
# - Then mutations Si to SC (max 200000 generations, and stop when invasion of SC)

function simulation(nbSim, Npop, alpha, delta, U_SI, U_SC, L, dominanceSC)
    #Creating output lists (for all runs)
    Tot_nbSalleles::Vector{Float64} = []
    Tot_ID::Vector{Float64} = []
    Tot_nbSC::Vector{Float64} = []
    Tot_freqSC::Vector{Float64} = []
    Tot_Dom::Vector{Vector{Float64}} = []
    Tot_DomFreq::Vector{Vector{Float64}} = []
    Tot_FreqAlleles::Vector{Vector{Float64}} = []
    
    threshold_mutation_filter = Npop*100000.0

    for simu in 1:nbSim
        nbSalleles = 0
        nbSCalleles = 0
        freqSC = 0.0
        ID = 0
        
        it = 0
        println("nbSimu=" , simu)
        #Creating intermediate lists
        list_ID::Vector{Float64}, list_nbSalleles::Vector{Int} = [],[]
        list_nbSC::Vector{Int} = []
        list_dom::Vector{Vector{Int}} = []
        list_domFreq::Vector{Vector{Float64}} = []
	list_allelesFreq::Vector{Vector{Float64}} = []
	


        #Initialisation
        nbS = 100 #number of possible S alleles
        list_ind::Vector{Vector{Vector{Float64}}} = []
        for i in 1:Npop
            ind::Vector{Vector{Float64}} = [[0.0,0.0], [],[]]
            push!(list_ind,ind)
            list_ind[i][1][1] = rand(0:nbS-1)
            list_ind[i][1][2] = rand(0:nbS-1) 
        end
        list_W = [1.0 for _ in 1:Npop]
        
        list_off::Vector{Vector{Vector{Float64}}} = []
        for _ in 1:Npop
            ind::Vector{Vector{Float64}} = [[0.0,0.0], [],[]]
            push!(list_off,ind)
        end
        list_W_off = Vector{Float64}(undef, Npop)


        generation = 0
        phase = 0
        
        max_gen = 250000

        #Phase 0: Starting with mutations on the S locus only (SI to SI)
        #Phase 1: Introduction of deleterious mutations on the genome
        #Phase 2: Introduction of S locus mutations to SC alleles
        
        while phase <= 2 
            generation += 1
            if generation%1000==0
                println("generation=", generation)
            end


            #Creating the next generation
            reproduction!(Npop, list_ind, list_W, list_off, list_W_off, alpha, delta, L, phase, dominanceSC)
            
            #Mutations
            mutation_SI!(Npop, list_off, U_SI, nbS) #on the S locus: SI to SI
            if phase >= 2
                mutation_SC!(Npop, list_off, U_SC) #on the S locus: SI to SC
            end


            #Computing Inbreeding depression and number of S alleles (in total and in dominance classes)
            if phase == 1 && generation > 2000 - 50
                list_alleles = []
                list_nbDom = [0,0,0,0]
		freq_alleles = [0.0 for _ in 1:nbS]
                for i in 1:Npop
                    S1, S2 = list_ind[i][1][1], list_ind[i][1][2]
		    freq_alleles[Int(S1)+1] += 1.0
                    freq_alleles[Int(S2)+1] += 1.0
                    append!(list_alleles,S1)
                    append!(list_alleles,S2)
                    domS1 = Int(S1÷25+1) #dominance class of first S allele
                    domS2 = Int(S2÷25+1) #dominance class of second S allele
                    list_nbDom[domS1]+=1
                    list_nbDom[domS2]+=1
                end
		freq_alleles = freq_alleles./(2*Npop)
                nS = length(unique(list_alleles))
                push!(list_nbSalleles,nS)
                push!(list_dom, list_nbDom)
                push!(list_allelesFreq, freq_alleles)
                
            end

            
            list_ind, list_off = list_off, list_ind
            list_W, list_W_off = list_W_off, list_W
 
            
            if phase == 0 && generation > 2000
                println("Going to phase 1")
                phase+= 1
                generation = 0
            end
            if phase == 1 && generation > 2000
                println("Going to phase 2")
                nbSalleles = sum(list_nbSalleles)/length(list_nbSalleles) #number of S alleles before introducing the SC allele averaged over the last 50 generations
                phase+= 1
                generation = 0
            end
            
            #Computing the number and frequency of SC alleles + Stopping criteria (Stop after 50,000 generations if SC is fixed in the population)
            if phase == 2 && generation%100==0 && generation > 50000
                nbSC = 0
                for i in 1:Npop
                    S1, S2 = list_ind[i][1][1], list_ind[i][1][2]
                    if S1 < 0
                        nbSC +=1
                    end
                    if S2 < 0
                        nbSC+=1
                    end
                end
                
                push!(list_nbSC,nbSC)

                #Stopping criteria
                if nbSC == 2*Npop #SC is fixed in the population
                    nbSCalleles = nbSC
                    freqSC = 1
                    phase +=1 #Stop the simulation
                end
            
                if generation > max_gen-150
                    nbSCalleles = sum(list_nbSC[end-299:end])/300 #average of the nbSalleles over the last 30000 generations
                    freqSC = nbSCalleles/(2*Npop)
                    phase += 1 #Stops the simulation
                end
            end
                
        end


        #Complete the list with what to return
        push!(Tot_nbSalleles,nbSalleles)
        push!(Tot_ID,delta)
        push!(Tot_nbSC,nbSCalleles)
        push!(Tot_freqSC,freqSC)
        vec_av_dom = [0.0, 0.0, 0.0, 0.0]
        for i in 1:length(vec_av_dom)
            vec_av_dom[i] = mean([ld[i] for ld in list_dom])
        end
        vec_freq_dom = vec_av_dom./sum(vec_av_dom)
        push!(Tot_Dom, vec_av_dom)
        push!(Tot_DomFreq, vec_freq_dom)

    	vec_av_alleles = [0.0 for _ in 1:nbS]
        for i in 1:length(vec_av_alleles)
                vec_av_alleles[i] = mean([la[i] for la in list_allelesFreq])
        end
        push!(Tot_FreqAlleles, vec_av_alleles)
        println("Done !")
    end
    
    return (Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_Dom, Tot_DomFreq, Tot_FreqAlleles)
end









#PARAMETERS

nbSim = 10

Npop = 1000
L = 10
alpha = 0.1 #Tested between 0 and 1
U_SC = 1e-4
U_SI = 1e-5
domSC = 1 #dominance of SC allele (1,2,3,4 or 5, 1 being the most recessive and 5 being the most dominant)
delta = 0.7 #Tested between 0 and 1


Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_Dom, Tot_DomFreq, Tot_FreqAlleles = simulation(nbSim, Npop, alpha, delta, U_SI, U_SC, L, domSC)


