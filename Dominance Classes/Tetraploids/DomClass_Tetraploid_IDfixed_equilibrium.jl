...
function selfing_rate(Npop, list_ind, par, alpha, list_W, dominanceSC)
    ...
    #Case 1,2 or 3 SC
        ...
        domM_SI_1, domM_SI_2, domM_SI_3, domM_SI_4 = -1,-1,-1,-1

        #Checking if the individual is SC
        if Salleles[1] != -1 
            domM_SI_1 = Salleles[1]÷25 + 1 #Dominance class of the SI S allele
        end
        if Salleles[2] != -1 
            domM_SI_2 = Salleles[2]÷25 + 1 #Dominance class of the SI S allele
        end
        if Salleles[3] != -1 
            domM_SI_3 = Salleles[3]÷25 + 1 #Dominance class of the SI S allele
        end
        if Salleles[4] != -1 
            domM_SI_4 = Salleles[4]÷25 + 1 #Dominance class of the SI S allele
        end

        list_domM = [domM_SI_1, domM_SI_2, domM_SI_3, domM_SI_4]
        for dom in list_domM
            if dom >= dominanceSC #The individual is SI, as SC is not the most dominant allele in the mother 
                return 0.0
            end
        end
        ...
        for j in 1:Npop
            ...
            if j != par
                SF = list_ind[j][1] #S alleles of the pollen
                SF_1 = SF[1]
                SF_2 = SF[2]
                SF_3 = SF[3]
                SF_4 = SF[4]

                #Dominance in the pollen
                domF_1 = SF_1÷25 + 1 #Dominance class of the first allele in the pollen
                domF_2 = SF_2÷25 + 1 
                domF_3 = SF_3÷25 + 1 
                domF_4 = SF_4÷25 + 1 
                if SF_1 == -1
                ...
                #Checking the compatibility of the pollen
                SF_Dom = [SF_1, SF_2, SF_3, SF_4]
                if SF_Dom[1] == -1 && SF_Dom[2] == -1 && SF_Dom[3] == -1 && SF_Dom[4] == -1 #The most dominant allele in the pollen is SC
                    gamma_ij = 2
                else
                    SF_SI = filter(!=(-1), SF_Dom)
                    nb_incomp = 0 
                    for i in 1:length(SF_SI)
                        if SF_SI[i] == Salleles[1] || SF_SI[i] == Salleles[2] || SF_SI[i] == Salleles[3] || SF_SI[i] == Salleles[4]
                            nb_incomp +=1
                        end
                    end
                    if nb_incomp == 0
...
function reproduction!(Npop, list_ind, list_W, list_off, list_W_off, alpha, delta, L, phase, dominanceSC)
    ...
    while Npop_off != Npop
        ...
        selfRate = selfing_rate(Npop, list_ind, par1, alpha, list_W, dominanceSC)
        OK_father = false
        fit_off = 1.0
        ...
        if selfRate <=  rand()
            ...
            while OK_father == false && loop < maxAttempts
                ...
                Salleles_par2 = list_ind[par2][1]
                Salleles_par2_1 = Salleles_par2[1]
                Salleles_par2_2 = Salleles_par2[2]
                Salleles_par2_3 = Salleles_par2[3]
                Salleles_par2_4 = Salleles_par2[4]

                #Dominance in the father
                domF_1 = Salleles_par2_1÷25 + 1
                domF_2 = Salleles_par2_2÷25 + 1
                domF_3 = Salleles_par2_3÷25 + 1
                domF_4 = Salleles_par2_4÷25 + 1
                if Salleles_par2_1 == -1
                    domF_1 = dominanceSC
                end
                if Salleles_par2_2 == -1
                    domF_2 = dominanceSC
                end
                if Salleles_par2_3 == -1
                    domF_3 = dominanceSC
                end
                if Salleles_par2_4 == -1
                    domF_4 = dominanceSC
                end
                
                #We artificially consider that all S alleles in the father are one of the most dominant one
                if domF_1 > domF_2
                    Salleles_par2_2 = Salleles_par2_1
                end
                if domF_1 > domF_4
                    Salleles_par2_4 = Salleles_par2_1
                end
                if domF_2 > domF_1
                    Salleles_par2_1 = Salleles_par2_2
                end
                if domF_2 > domF_3
                    Salleles_par2_3 = Salleles_par2_2
                end
                if domF_3 > domF_4
                    Salleles_par2_4 = Salleles_par2_3
                end
                if domF_3 > domF_2
                    Salleles_par2_2 = Salleles_par2_3
                end
                if domF_4 > domF_3
                    Salleles_par2_3 = Salleles_par2_4
                end
                if domF_4 > domF_1
                    Salleles_par2_1 = Salleles_par2_4
                end
                if domF_1 > domF_3
                    Salleles_par2_3 = Salleles_par2_1
                end
                if domF_3 > domF_1
                    Salleles_par2_1 = Salleles_par2_3
                end
                if domF_2 > domF_4
                    Salleles_par2_4 = Salleles_par2_2
                end
                if domF_4 > domF_2
                    Salleles_par2_2 = Salleles_par2_4
                end


                #Checking the compatibility of the pollen
                Salleles_par2_Dom = [Salleles_par2_1, Salleles_par2_2, Salleles_par2_3, Salleles_par2_4]
                if Salleles_par1[1] == -1 && Salleles_par1[2] == -1 && Salleles_par1[3] == -1 && Salleles_par1[4] == -1 #All alleles in the mother are SC
                    OK_father = true
                    break
                end
                if Salleles_par2_Dom[1] == -1 && Salleles_par2_Dom[2] == -1 && Salleles_par2_Dom[3] == -1 && Salleles_par2_Dom[4] == -1 #The most dominant allele in the pollen is SC
                    OK_father = true
                    break
                else
                    Salleles_par2_SI = filter(!=(-1), Salleles_par2_Dom)
                    nb_incomp = 0 
                    for i in 1:length(Salleles_par2_SI)
                        if Salleles_par2_SI[i] == Salleles_par1[1] || Salleles_par2_SI[i] == Salleles_par1[2] || Salleles_par2_SI[i] == Salleles_par1[3] || Salleles_par2_SI[i] == Salleles_par1[4]
                            nb_incomp +=1
                        end
                    end
                    if nb_incomp == 0
...
# - Nothing more here when delta is fixed
# - Then mutations Si to SC (max 200000 generations, and stop when invasion of SC)

function simulation(nbSim, Npop, alpha, delta, U_SI, U_SC, L, dominanceSC)
    ...
    Tot_Dom::Vector{Vector{Float64}} = []
    Tot_DomFreq::Vector{Vector{Float64}} = []
    Tot_FreqAlleles::Vector{Vector{Float64}} = []
    ...
    for simu in 1:nbSim
        ...
        list_ID::Vector{Float64} =[]
        list_nbSalleles::Vector{Int} = []
        list_nbSC::Vector{Int} = []
        list_dom::Vector{Vector{Int}} = []
        list_domFreq::Vector{Vector{Float64}} = []
    	list_allelesFreq::Vector{Vector{Float64}} = []
        ...
        #Phase 1: Introduction of deleterious mutations on the genome (none here)
        ...
        while phase <= 2 
            ...
            reproduction!(Npop, list_ind, list_W, list_off, list_W_off, alpha, delta, L, phase, dominanceSC)
            ...
            if phase == 1 && generation > 2000 - 50
                list_alleles = []
                list_nbDom = [0,0,0,0]
                freq_alleles = [0.0 for _ in 1:nbS]
                for i in 1:Npop
                    S1, S2, S3, S4 = list_ind[i][1][1], list_ind[i][1][2], list_ind[i][1][3], list_ind[i][1][4]
                    freq_alleles[Int(S1)+1] += 1.0
                    freq_alleles[Int(S2)+1] += 1.0
                    freq_alleles[Int(S3)+1] += 1.0
                    freq_alleles[Int(S4)+1] += 1.0
                    append!(list_alleles,S1)
                    append!(list_alleles,S2)
                    append!(list_alleles,S3)
                    append!(list_alleles,S4)
                    domS1 = Int(S1÷25+1) #dominance class of first S allele
                    domS2 = Int(S2÷25+1)
                    domS3 = Int(S3÷25+1) 
                    domS4 = Int(S4÷25+1)
                    list_nbDom[domS1]+=1
                ...
                freq_alleles = freq_alleles./(4*Npop)
                nS = length(unique(list_alleles))
                push!(list_nbSalleles,nS)
                push!(list_dom, list_nbDom)
                push!(list_allelesFreq, freq_alleles)
            end
        ...
        push!(Tot_ID, delta)
        vec_av_dom = [0.0, 0.0, 0.0, 0.0]
        for i in 1:length(vec_av_dom)
            vec_av_dom[i] = mean([ld[i] for ld in list_dom])
        end
        vec_freq_dom = vec_av_dom./sum(vec_av_dom)
        push!(Tot_Dom, vec_av_dom)
        push!(Tot_DomFreq, vec_freq_dom)
        vec_av_alleles = [0.0 for _ in 1:nbS]
        for i in 1:length(vec_av_alleles)
            vec_av_alleles[i] = mean([la[i] for la in list_allelesFreq])
        end
        push!(Tot_FreqAlleles, vec_av_alleles)
        println("Done !")
        
    end
    
    return (Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_DomFreq, Tot_FreqAlleles)
end
...
U_SC = 1e-4
U_SI = 1e-5
dom = 5 #dominance of SC allele (1,2,3,4 or 5, 1 being the most recessive and 5 being the most dominant)
delta = 0.5 #Tested between 0 and 1


Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_DomFreq, Tot_FreqAlleles = simulation(nbSim, Npop, alpha, delta, U_SI, U_SC, L, dom)
