...
function recombination(par, L)
    ...
    return (Sallele_chr1, Sallele_chr2, [], [])
end
    ...
    #Case 1,2 or 3 SC
        ...
        for j in 1:Npop
            ...
            if j != par
                ...
                if SF_1 == -1 && SF_2 == -1 && SF_3 == -1 && SF_4 == -1 #The most dominant allele in the pollen is SC
                    gamma_ij = 2
                else
                    nb_incomp = 0 
                    for s in (SF_1, SF_2, SF_3, SF_4)
                        if s != -1
                            if s == Salleles[1] || s == Salleles[2] || s == Salleles[3] || s == Salleles[4]
                                nb_incomp += 1
                            end
                        end
                    end
    ...
    while Npop_off != Npop
        ...
        if selfRate <=  rand()
            ...
            while OK_father == false && loop < maxAttempts
                ...
                if Salleles_par2_1 == -1 && Salleles_par2_2 == -1 && Salleles_par2_3 == -1 && Salleles_par2_4 == -1 #The most dominant allele in the pollen is SC
                    OK_father = true
                    break
                else
                    nb_incomp = 0 
                    for s in (Salleles_par2_1, Salleles_par2_2, Salleles_par2_3, Salleles_par2_4)
                        if s != -1
                            if s == Salleles_par1[1] || s == Salleles_par1[2] || s == Salleles_par1[3] || s == Salleles_par1[4]
                                nb_incomp += 1
                            end
                        end
                    end
...
function simulation(nbSim, Npop, alpha, U_SI, U_SC, L, dominanceSC)
...
U_SC = 1e-4
U_SI = 1e-5
dom = 3 #dominance of SC allele (1,2,3,4 or 5, 1 being the most recessive and 5 being the most dominant)


Tot_nbSalleles, Tot_ID, Tot_nbSC, Tot_freqSC, Tot_DomFreq, Tot_FreqAlleles = simulation(nbSim, Npop, alpha, U_SI, U_SC, L, dom)
